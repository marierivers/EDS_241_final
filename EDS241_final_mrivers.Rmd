...
data <- read_csv(here("data", "KM_EDS241.csv")) %>% 
  ...
  mutate(year = as.factor(year)) %>% # xxx...run through all code to make sure this doesn't mess up other questions
...
```{r}
# xxx...not sure which model to use, but they seem to give the same result
...
#                index = c("year"), model = "within", effect = "twoways", data = data)
# DD
```

### Answer d:
\noindent Based on the difference-in-difference (DD) estimator, the causal effect of the incinerator on housing values without controlling for house and lot characteristics is $`r DD_model_d_coef`. The sign of the estimated DD coefficient is negative and that mean that houses for the treatment group (near the incinerator) have a lower price than houses away from the incinerator. The magnitude of this estimator means that the difference between the two groups is \$`r abs(DD_model_d_coef)`.  Based on the p-value, the DD coefficient is not statistically significant. Between 1978 and 1981, house values did not change more for houses near the incinerator than they did for houses not near the incinerator.
...
xxx...difference in price from being near the incinerator in 1981 vs. being near the incinerator in 1978.
...
```{r}
# DD_model_f <- plm(rprice ~ nearinc + age + rooms + area + land, 
#                index = c("year"), model = "within", effect = "twoways", data = data)
# DD_model_f
```
...
### Answer g:
\noindent On average, real housing values increased by $`r DD_model_f_coef_control` between 1978 and 1981 for houses not near the incinerator.
